...
The overarching goal of this study is to define the wave energy resources and to develop a consistent and accurate methodology to estimate the total theoretically available wave resource for a given region. This methodology has been guided by two core principals:
\begin{itemize}
\item Comprehensive and accurate (include all available wave energy in a region, without double counting) \note{better words than `comp + accurate'?}
\item Utilize well-defined boundaries to define resource areas
\end{itemize}

There are four core-components to the methodology proposed here that -- taken together -- resolve many of the critiques that have been levelled against previous approaches:
\begin{itemize}
\item Utilize a one-way dot-product (as proposed previously by others, but not used in the most recent U.S. assessment; \cite{EPRIwaveresource2011}).
\item Extend the assessment to the edge of the U.S. EEZ. This has been done previously by some, but we believe that it should be done consistently for regional/national RA. ... in order to get consistent/comparable results.
\item Include `local' winds within a nation's EEZ (this is new!)
\item Don't count fluxes across {\it borders}
\end{itemize}
